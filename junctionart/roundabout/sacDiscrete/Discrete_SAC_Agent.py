...
from junctionart.roundabout.sacDiscrete.utilities.Network import Network
from junctionart.roundabout.sacDiscrete.utilities.ReplayBuffer import ReplayBuffer
...
class SACAgent:
    ...
    def __init__(self, environment, batch_size = 100):
        self.REPLAY_BUFFER_BATCH_SIZE = batch_size
        self.environment = environment
        self.state_dim = self.environment.state_dim
        self.action_dim = self.environment.num_actions - 1 # no need terminate
    ...
    def get_actions_in_batches(self, state_tensor):
        action_probabilities = self.actor_local.forward(state_tensor)
        # sample from the action probabilities
        action_distribution = torch.distributions.Categorical(action_probabilities)
        discrete_actions = action_distribution.sample()
    ...
    def train_on_transition(self, state, discrete_action, next_state, reward, done):
        ...
        # print(state, discrete_action, next_state)
